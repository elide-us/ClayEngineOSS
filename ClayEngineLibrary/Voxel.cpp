...
uint8_t ClayEngine::Voxel::EncodeVoxelVector(float vector)
{
	// (double + .5) / (2.5 / 256)
...
float ClayEngine::Voxel::DecodeVoxelVector(uint8_t vector)
{
	// (char * (2.5 / 256)) - .5
	return float((vector * c_vector_pack_ratio) - std::abs(c_voxel_vector_min));
}

void ClayEngine::Voxel::SetVoxel(float& z, float& y, float& x, unsigned int& lod)
{
...
void ClayEngine::Voxel::GetVoxel(float& z, float& y, float& x, unsigned int& lod)
{
...
ClayEngine::Voxel::Voxel(float z, float y, float x, unsigned int lod)
{
	m_z = EncodeVoxelVector(z);
	m_y = EncodeVoxelVector(y);
	m_x = EncodeVoxelVector(x);
...
ClayEngine::Voxel::operator uint32_t()
{
